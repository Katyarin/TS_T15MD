...
def writing_data(N_pages_get, shot_N):
    for N_pages in range(0, N_pages_get + 50, 50):
        ...
        if N_pages_get - N_pages > 50:
            with open('Files/' + start_options["data"] + '/' + str(shot_N) + '_' + str(N_pages) + '_to_' + str(N_pages + 50) + '.json', 'w') as fp:
                json.dump(ret_data, fp)
        elif 50 > N_pages_get - N_pages > 0:
            with open('Files/' + start_options["data"] + '/' + str(shot_N) + '_' + str(N_pages) + '_to_' + str(N_pages_get - N_pages) + '.json',
                      'w') as fp:
                json.dump(ret_data, fp)
